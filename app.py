...
if uploaded_file is not None:
    ...
    <div class="status-card-red">
        ...
        <h3 style="margin-top:10px; color:#800000;">البند 04 (صفحة 12): آجال تحضير العروض</h3>
        <p><strong>التحليل الآلي:</strong> تم تحديد أجل تحضير العروض بـ 15 يوماً فقط، بينما ينص التشريع على ألا يقل الأجل عن 21 إلى 30 يوماً لضمان المنافسة الشريفة وتكافؤ الفرص.</p>
        ...
        <p style="color:#d32f2f;"><strong>التوصية تعديل البند:</strong> تعديل الأجل فوراً ليكون 21 يوماً على الأقل لتفادي طعن المتنافسين وإلغاء الصفقة.</p>
    </div>
    """, unsafe_allow_html=True)

    st.markdown("""
    <div class="status-card-orange">
        <span class="badge-orange">تنبيه تنظيمي (غموض ومعايير التأهيل)</span>
        <h3 style="margin-top:10px; color:#b7791f;">البند 02 (صفحة 03): غموض معايير التأهيل المالي</h3>
        <p><strong>التحليل الآلي:</strong> عدم تحديد حد أدنى لرقم الأعمال السنوي المطلوب، مما يخالف تعليمات وزارة المالية ويفتح باب التأويل والمحاباة وعدم تكافؤ الفرص.</p>
    ...
    <div class="status-card-orange">
        ...
        <hr style="margin:10px 0;">
        <p style="color:#b7791f;"><strong>التوصية تعديل البند:</strong> إدراج بند تخصيص المؤسسات الناشئة والمصغرة طبقاً للتعليمات.</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    # محتوى التقرير النصي الرسمي للتحميل المباشر
    report_text = f"""
==================================================
RESEEN LEGAL AUDIT - OFFICIAL REPORT
==================================================
Project Name: Raseen AI - Automated Legal Auditing
Target: TRL 4 Prototype for Protomarket II
Document Audited: {uploaded_file.name}
...
==================================================
    ...
    st.download_button(
        label="📥 (رسمي TXT/PDF) تحميل تقرير التدقيق القانوني الشامل",
        data=report_text,
        file_name="Raseen_Audit_Report.txt",
        mime="text/plain"
    )
